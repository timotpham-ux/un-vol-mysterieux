...
/*
  État temporaire utilisé lorsqu'une carte ouvre un fichier audio ou vidéo.
  La musique d'ambiance est interrompue, puis reprise lorsque le joueur
  revient réellement sur la page du jeu.
*/
let ambienceInterruptedForMedia = false;
let pageLostFocusAfterMedia = false;
let mediaOpenedAt = 0;
...
function isMediaDestination(destination) {
  ...
  /*
    On retire les paramètres et les ancres afin de reconnaître aussi :
    sons/scene.mp3?v=2
    videos/fin.mp4#lecture
  */
  ...
  return mediaExtensions.some((extension) =>
    cleanDestination.endsWith(extension)
  );
}
...
function interruptAmbienceForMedia() {
  stopCrossfade();
  pauseAllAudio();

  /*
    On conserve audioEnabled à true : le bouton audio reste activé et
    la piste pourra reprendre automatiquement au retour sur cette page.
  */
  ambienceInterruptedForMedia = true;
  pageLostFocusAfterMedia = false;
  mediaOpenedAt = Date.now();

  setMessage(
    "Ambiance mise en pause pendant la lecture du fichier audio ou vidéo.",
    true
  );
}

async function resumeAmbienceAfterMedia() {
  if (!ambienceInterruptedForMedia) {
    return;
  }

  ambienceInterruptedForMedia = false;
  pageLostFocusAfterMedia = false;
  ...
  setMessage("Ambiance sonore reprise.", true);
}
...
function validateCode() {
  ...
  /*
    Un numéro de carte peut servir à ouvrir un fichier, changer
    l'ambiance ou sélectionner une carte d'indice.
  */
  if (destination || soundtrackKey || hasHints) {
    ...
    const opensMedia = isMediaDestination(destination);

    /*
      Priorité au média :
      - si la carte ouvre un son ou une vidéo, l'ambiance s'arrête ;
      - sinon, une éventuelle nouvelle ambiance est lancée.
    */
    if (opensMedia) {
      interruptAmbienceForMedia();
    ...
    /*
      Pour un fichier audio/vidéo, on n'ajoute pas le bruitage de validation,
      afin de ne pas le superposer au début du média.
    */
    if (!opensMedia) {
      playEffect("correct");
    }
    ...
    if (destination) {
      setMessage(
        opensMedia
          ? `Carte ${code} validée : ambiance interrompue pendant le média.`
          : `Carte ${code} validée.`,
        true
      );
      ...
      if (!openedWindow) {
        /*
          Si le navigateur bloque la fenêtre, on annule l'interruption,
          car aucun média n'a été ouvert.
        */
        if (opensMedia) {
          ambienceInterruptedForMedia = false;
          pageLostFocusAfterMedia = false;
        ...
        setMessage(
          "Le navigateur a bloqué l'ouverture. Autorisez les fenêtres surgissantes.",
          false
        );
      }
    } else if (soundtrackKey) {
...
async function changeSoundtrack(key) {
  /*
    Une carte qui change explicitement l'ambiance annule l'attente
    liée à un ancien média.
  */
  ambienceInterruptedForMedia = false;
  pageLostFocusAfterMedia = false;
...
/*
  Lorsqu'un fichier média est ouvert dans un nouvel onglet, la page du jeu
  perd généralement le focus ou devient cachée. La musique reprend seulement
  au véritable retour du joueur, et non immédiatement après window.open().
*/
window.addEventListener("blur", () => {
  if (ambienceInterruptedForMedia) {
    pageLostFocusAfterMedia = true;
  }
});

document.addEventListener("visibilitychange", () => {
  if (ambienceInterruptedForMedia && document.hidden) {
    pageLostFocusAfterMedia = true;
  }

  if (
    ambienceInterruptedForMedia &&
    !document.hidden &&
    pageLostFocusAfterMedia &&
    Date.now() - mediaOpenedAt > 500
  ) {
    resumeAmbienceAfterMedia().catch(() => {});
  }
});
